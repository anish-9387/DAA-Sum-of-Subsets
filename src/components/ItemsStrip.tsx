...
function ItemsStrip({
  ...
  const includedSet = new Set<number>(includedIndexes)
  ...
  return (
    <div className="flex flex-wrap gap-3">
      {items.map((item, index) => {
        ...
        const baseClasses =
          'rounded-xl border px-3 py-2 text-sm font-semibold transition duration-300'
        ...
        const consideringClasses = considering ? 'ring-2 ring-amber-300/90 scale-105' : ''

        return (
          <div
            key={`${item}-${index}`}
            className={`${baseClasses} ${stateClasses} ${consideringClasses}`}
          >
            Rs {item}
          </div>
        )
